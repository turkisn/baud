...
import { BadgeCheck, Building2, Filter, MapPin, Palette, Ruler, Search, SlidersHorizontal } from 'lucide-react';
...
import { collectPages, filterCatalogProducts } from '../utils/catalog';
...
export default function Blocks() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      collectPages((page) => mvpService.getSuppliers(page)),
  ...
  const matchingProducts = filterCatalogProducts(products, { city, material, color, dimension }, suppliers);
  const visibleProducts = matchingProducts.slice(0, visibleLimit);
  const hasMore = visibleLimit < matchingProducts.length;
    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      {loading ? <LoadingState/> : products.length === 0 && error ? <ErrorState/> : visibleProducts.length === 0 ? <EmptyState title={t('No blocks found', 'لا توجد بلوكات')} description={t('Try a different search or category. New published blocks will appear here.', 'جرّب بحثاً أو فئة مختلفة. ستظهر البلوكات المنشورة الجديدة هنا.')}/> : <>
        ...
        <div className="mt-10 flex flex-col items-center gap-3">
          {hasMore ? <button type="button" onClick={() => setVisibleLimit((current) => current + PAGE_SIZE)} className="btn-primary min-w-44 justify-center">{t('Load more blocks', 'تحميل المزيد من البلوكات')}</button> : <p className="text-sm text-light-brown">{t('You have reached the end of the matching blocks.', 'وصلت إلى نهاية البلوكات المطابقة.')}</p>}
        </div>
      </>}
    </div>
  </div>;
