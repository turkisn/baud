...
function supplierEditorState(record = {}) {
  const state = { ...EMPTY_SUPPLIER, ...record };
  for (const [field, fallback] of Object.entries(EMPTY_SUPPLIER)) {
...
function SupplierEditor({ initial, onClose, onSaved, t }) {
  ...
  const [supplier, setSupplier] = useState(() => supplierEditorState(initial));
  ...
  const save = async (isPublished) => {
    ...
    if (!validHttpsUrl(supplier.website)) {
      setError(t('Website must use a secure https URL.', 'يجب أن يستخدم الموقع عنوان https آمناً.'));
      return;
    }
    ...
    try {
      ...
      setSupplier(supplierEditorState(fresh));
  ...
  const uploadAsset = async (kind) => {
    ...
    try {
      ...
      registered = true;
      setSupplier(supplierEditorState(next));
      const fresh = await mvpAdminService.getSupplier(saved?.id || supplier.id);
      setSupplier(supplierEditorState({ ...next, ...fresh }));
  ...
  return <div ref={dialogRef} tabIndex="-1" className="fixed inset-0 z-50 overflow-y-auto bg-deep-brown/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={t('Supplier editor', 'محرر المورد')}><div className="mx-auto max-w-5xl overflow-hidden rounded-3xl bg-ivory shadow-2xl"><header className="sticky top-0 z-10 flex items-center justify-between border-b border-sand bg-white px-5 py-4"><div><p className="text-xs font-bold uppercase tracking-wider text-gold">{supplier.id ? t('Edit Supplier Window', 'تعديل نافذة المورد') : t('New Supplier Window', 'نافذة مورد جديدة')}</p><h2 className="mt-1 text-xl font-black text-dark-brown">{supplier.company_name_en || t('Untitled supplier', 'مورد بدون عنوان')}</h2></div><button ref={initialFocusRef} type="button" onClick={onClose} aria-label={t('Close editor', 'إغلاق المحرر')} className="admin-icon-button"><X/></button></header>
    <div className="space-y-6 p-5 sm:p-7">{error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-700">{error}</div>}{success && <div role="status" className="flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 p-4 text-sm font-medium text-green-700"><CheckCircle2 size={17}/>{success}</div>}
      <section className="rounded-2xl border border-sand bg-white p-5"><h3 className="mb-4 font-bold text-dark-brown">{t('Supplier data', 'بيانات المورد')}</h3><div className="grid gap-4 md:grid-cols-2"><Field label="Company name (English)" required value={supplier.company_name_en} onChange={set('company_name_en')}/><Field label="اسم الشركة (العربية)" required dir="rtl" value={supplier.company_name_ar} onChange={set('company_name_ar')}/><Field label="Slug" value={supplier.slug} onChange={set('slug')}/><Field label={t('Commercial name', 'الاسم التجاري')} value={supplier.commercial_name} onChange={set('commercial_name')}/><Textarea label="Description (English)" value={supplier.description_en} onChange={set('description_en')}/><Textarea label="الوصف (العربية)" dir="rtl" value={supplier.description_ar} onChange={set('description_ar')}/><Field label={t('Website', 'الموقع الإلكتروني')} type="url" inputMode="url" placeholder="https://" value={supplier.website} onChange={set('website')} className="md:col-span-2"/><Field label={t('Country', 'الدولة')} value={supplier.country} onChange={set('country')}/><Field label={t('City', 'المدينة')} value={supplier.city} onChange={set('city')}/><Field label={t('Sort order', 'ترتيب العرض')} type="number" value={supplier.sort_order} onChange={set('sort_order')}/><label className="block"><span className="mb-1.5 block text-xs font-semibold text-medium-brown">{t('Verification status', 'حالة التحقق')}</span><select value={supplier.verification_status} onChange={set('verification_status')} className="input-field bg-white"><option value="unverified">{t('Unverified', 'غير موثق')}</option><option value="pending">{t('Pending', 'قيد المراجعة')}</option><option value="verified">{t('Verified', 'موثق')}</option></select></label></div></section>
      <section className="rounded-2xl border border-sand bg-white p-5"><div className="mb-4"><h3 className="font-bold text-dark-brown">{t('Private supplier assets', 'ملفات المورد الخاصة')}</h3><p className="mt-1 text-xs text-light-brown">{t('Save the supplier draft first. Logo and cover images may be up to 10 MB and use short-lived signed reads.', 'احفظ مسودة المورد أولاً. يمكن أن يصل حجم صور الشعار والغلاف إلى 10 ميجابايت وتستخدم قراءة موقعة مؤقتاً.')}</p></div><div className="grid gap-5 md:grid-cols-2"><div className="rounded-xl border border-sand bg-warm-white p-4"><div className="mb-3 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-xl bg-white">{supplier.signed_logo_url ? <img src={supplier.signed_logo_url} alt={t('Supplier logo preview', 'معاينة شعار المورد')} className="h-full w-full object-contain"/> : <Image aria-hidden="true" className="text-beige" size={34}/>}</div><label className="mb-2 block text-xs font-semibold text-medium-brown">{t('Logo image', 'صورة الشعار')}</label><input aria-label={t('Choose a supplier logo, up to 10 MB', 'اختر شعار المورد، حتى 10 ميجابايت')} type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => setLogoFile(event.target.files?.[0] || null)} disabled={!supplier.id || Boolean(saving)} className="block w-full text-xs text-medium-brown"/><button type="button" onClick={() => uploadAsset('logo')} disabled={!supplier.id || !logoFile || Boolean(saving)} className="admin-secondary-button mt-3">{saving === 'logo' ? <Loader2 className="animate-spin" size={14}/> : <Upload size={14}/>} {t('Upload logo', 'رفع الشعار')}</button></div><div className="rounded-xl border border-sand bg-warm-white p-4"><div className="mb-3 flex aspect-[16/9] items-center justify-center overflow-hidden rounded-xl bg-white">{supplier.signed_cover_url ? <img src={supplier.signed_cover_url} alt={t('Supplier cover preview', 'معاينة غلاف المورد')} className="h-full w-full object-cover"/> : <Image aria-hidden="true" className="text-beige" size={34}/>}</div><label className="mb-2 block text-xs font-semibold text-medium-brown">{t('Cover image', 'صورة الغلاف')}</label><input aria-label={t('Choose a supplier cover image, up to 10 MB', 'اختر صورة غلاف المورد، حتى 10 ميجابايت')} type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => setCoverFile(event.target.files?.[0] || null)} disabled={!supplier.id || Boolean(saving)} className="block w-full text-xs text-medium-brown"/><button type="button" onClick={() => uploadAsset('cover')} disabled={!supplier.id || !coverFile || Boolean(saving)} className="admin-secondary-button mt-3">{saving === 'cover' ? <Loader2 className="animate-spin" size={14}/> : <Upload size={14}/>} {t('Upload cover', 'رفع الغلاف')}</button></div></div></section>
    </div>
    <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t border-sand bg-white px-5 py-4"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${supplier.is_published ? 'bg-green-100 text-green-800' : 'bg-sand text-medium-brown'}`}>{supplier.is_published ? t('Published', 'منشور') : t('Unpublished', 'غير منشور')}</span>{supplier.verification_status && <VerifyBadge status={supplier.verification_status}/>} {!canPublish && <span id="supplier-publish-requirements" className="text-xs text-amber-700">{t('Publishing requires both company names and a valid website link when provided.', 'يتطلب النشر اسمي الشركة ورابط موقع صالحاً عند إدخاله.')}</span>}</div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => save(false)} disabled={Boolean(saving)} className="admin-secondary-button">{saving === 'draft' ? <Loader2 className="animate-spin" size={14}/> : <Save size={14}/>} {supplier.id ? t('Save / unpublish', 'حفظ / إلغاء النشر') : t('Save draft', 'حفظ كمسودة')}</button><button type="button" onClick={() => save(true)} disabled={Boolean(saving) || !canPublish} aria-describedby={!canPublish ? 'supplier-publish-requirements' : undefined} className="admin-primary-button disabled:cursor-not-allowed disabled:opacity-50">{saving === 'publish' ? <Loader2 className="animate-spin" size={14}/> : <Send size={14}/>} {t('Publish', 'نشر')}</button></div></div>
  </div></div>;
}
