...
function productEditorState(record = {}) {
  const state = { ...EMPTY_PRODUCT, ...record };
  for (const [field, fallback] of Object.entries(EMPTY_PRODUCT)) {
    state[field] = record[field] ?? fallback;
  }
  return {
    ...state,
    price: record.price ?? '',
...
function ProductAssets({ product, onRefresh, disabled, t }) {
  ...
  return <section className="space-y-5 rounded-2xl border border-sand bg-white p-5"><div><h3 className="font-bold text-dark-brown">{t('Private product assets', 'ملفات المنتج الخاصة')}</h3><p className="mt-1 text-xs text-light-brown">{t('Uploads use private buckets. Public visitors only receive short-lived signed image reads and safe file metadata.', 'تستخدم عمليات الرفع حاويات خاصة. يحصل الزوار فقط على صور موقعة مؤقتاً وبيانات ملفات آمنة.')}</p></div>
    {error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}{success && <div role="status" className="rounded-xl border border-green-200 bg-green-50 p-3 text-sm text-green-700">{success}</div>}
    <div className="grid gap-5 xl:grid-cols-2">
      <form aria-label={t('Upload a product image', 'رفع صورة منتج')} onSubmit={uploadImage} className="space-y-3 rounded-xl border border-sand bg-warm-white p-4">
        <div className="flex items-center gap-2 font-semibold text-dark-brown"><Image size={17} className="text-gold"/>{t('Product image', 'صورة المنتج')}</div>
        <input aria-label={t('Choose a product image, up to 20 MB', 'اختر صورة المنتج، حتى 20 ميجابايت')} type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => setImageFile(event.target.files?.[0] || null)} disabled={disabled || uploading} className="block w-full text-xs text-medium-brown"/>
        <div className="grid gap-2 sm:grid-cols-2">
          <input aria-label={t('English image alternative text', 'النص البديل للصورة بالإنجليزية')} value={imageMeta.alt_text_en} onChange={(event) => setImageMeta((meta) => ({ ...meta, alt_text_en: event.target.value }))} placeholder="Alt text EN" className="input-field bg-white" disabled={disabled || Boolean(uploading)}/>
          <input aria-label={t('Arabic image alternative text', 'النص البديل للصورة بالعربية')} value={imageMeta.alt_text_ar} onChange={(event) => setImageMeta((meta) => ({ ...meta, alt_text_ar: event.target.value }))} placeholder="النص البديل AR" className="input-field bg-white" disabled={disabled || Boolean(uploading)}/>
          <select aria-label={t('Image type', 'نوع الصورة')} value={imageMeta.image_type} onChange={(event) => setImageMeta((meta) => ({ ...meta, image_type: event.target.value }))} className="input-field bg-white" disabled={disabled || Boolean(uploading)}><option value="render">Render</option><option value="dimension">Dimension</option><option value="material">Material</option><option value="catalogue">Catalogue</option><option value="lifestyle">Lifestyle</option><option value="other">Other</option></select>
          <input aria-label={t('Image sort order', 'ترتيب عرض الصورة')} type="number" value={imageMeta.sort_order} onChange={(event) => setImageMeta((meta) => ({ ...meta, sort_order: event.target.value }))} placeholder="Sort order" className="input-field bg-white" disabled={disabled || Boolean(uploading)}/>
        </div>
        <label className="flex items-center gap-2 text-xs text-medium-brown"><input type="checkbox" checked={imageMeta.is_primary} onChange={(event) => setImageMeta((meta) => ({ ...meta, is_primary: event.target.checked }))} disabled={disabled || Boolean(uploading)}/>{t('Register as primary image', 'تسجيل كصورة رئيسية')}</label>
        <button type="submit" disabled={!imageFile || disabled || uploading} className="admin-primary-button">{uploading === 'image' ? <Loader2 className="animate-spin" size={14}/> : <Upload size={14}/>} {t('Upload image', 'رفع الصورة')}</button>
      </form>
      <form aria-label={t('Upload a product file', 'رفع ملف منتج')} onSubmit={uploadFile} className="space-y-3 rounded-xl border border-sand bg-warm-white p-4">
        <div className="flex items-center gap-2 font-semibold text-dark-brown"><FileBox size={17} className="text-gold"/>{t('BIM / 3D or datasheet', 'ملف BIM / 3D أو ورقة بيانات')}</div>
        <select aria-label={t('Product file type', 'نوع ملف المنتج')} value={fileMeta.file_type} onChange={(event) => { setAssetFile(null); setFileMeta((meta) => ({ ...meta, file_type: event.target.value })); }} className="input-field bg-white" disabled={disabled || Boolean(uploading)}><option value="block">BIM / 3D block</option><option value="datasheet">Datasheet PDF</option></select>
        <input aria-label={t('Choose a product file', 'اختر ملف المنتج')} key={fileMeta.file_type} type="file" accept={fileMeta.file_type === 'datasheet' ? '.pdf,application/pdf' : BLOCK_FILE_ACCEPT} onChange={(event) => { const file = event.target.files?.[0] || null; setAssetFile(file); if (file && fileMeta.file_type === 'block') { const format = file.name.split('.').pop()?.toUpperCase(); if (PRODUCT_FILE_FORMATS.includes(format)) setFileMeta((meta) => ({ ...meta, file_format: format })); } }} disabled={disabled || uploading} className="block w-full text-xs text-medium-brown"/>
        <p className="text-[11px] leading-5 text-light-brown">{fileMeta.file_type === 'datasheet' ? t('PDF only, up to 20 MB.', 'PDF فقط، بحد أقصى 20 ميجابايت.') : t(`${PRODUCT_FILE_FORMATS.join(', ')} · up to 100 MB.`, `${PRODUCT_FILE_FORMATS.join('، ')} · بحد أقصى 100 ميجابايت.`)}</p>
        {fileMeta.file_type === 'block' && <div className="grid gap-2 sm:grid-cols-3">
          <select aria-label={t('File format', 'صيغة الملف')} value={fileMeta.file_format} onChange={(event) => setFileMeta((meta) => ({ ...meta, file_format: event.target.value }))} className="input-field bg-white" disabled={disabled || Boolean(uploading)}>{PRODUCT_FILE_FORMATS.map((format) => <option key={format}>{format}</option>)}</select>
          <input aria-label={t('Software name', 'اسم البرنامج')} value={fileMeta.software_name} onChange={(event) => setFileMeta((meta) => ({ ...meta, software_name: event.target.value }))} placeholder="Software" className="input-field bg-white" disabled={disabled || Boolean(uploading)}/>
          <input aria-label={t('Software version', 'إصدار البرنامج')} value={fileMeta.software_version} onChange={(event) => setFileMeta((meta) => ({ ...meta, software_version: event.target.value }))} placeholder="Version" className="input-field bg-white" disabled={disabled || Boolean(uploading)}/>
        </div>}
        <label className="flex items-center gap-2 text-xs text-medium-brown"><input type="checkbox" checked={fileMeta.is_primary} onChange={(event) => setFileMeta((meta) => ({ ...meta, is_primary: event.target.checked }))} disabled={disabled || Boolean(uploading)}/>{t('Register as primary file', 'تسجيل كملف رئيسي')}</label>
        <button type="submit" disabled={!assetFile || disabled || uploading} className="admin-primary-button">{uploading === 'file' ? <Loader2 className="animate-spin" size={14}/> : <Upload size={14}/>} {t('Upload file', 'رفع الملف')}</button>
      </form>
    </div>
...
function ProductEditor({ initial, categories, supplierOptions, onClose, onSaved, t }) {
  ...
  const save = async (publicationState) => {
    ...
    if (!validHttpsUrl(product.supplier_product_url) || !validHttpsUrl(product.source_url)) {
      setError(t('Supplier and source links must use secure https URLs.', 'يجب أن تستخدم روابط المورد والمصدر عناوين https آمنة.'));
      return;
    }
  ...
  const canPublish = Boolean(
    ...
    && validHttpsUrl(product.supplier_product_url)
    && validHttpsUrl(product.source_url)
  );

  return <div ref={dialogRef} tabIndex="-1" className="fixed inset-0 z-50 overflow-y-auto bg-deep-brown/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={t('Block editor', 'محرر البلوك')}><div className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-ivory shadow-2xl"><header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-sand bg-white px-5 py-4"><div><p className="text-xs font-bold uppercase tracking-wider text-gold">{product.id ? t('Edit block', 'تعديل البلوك') : t('New block', 'بلوك جديد')}</p><h2 className="mt-1 text-xl font-black text-dark-brown">{product.product_name_en || t('Untitled product', 'منتج بدون عنوان')}</h2>{product.buod_reference && <p className="mt-1 font-mono text-xs text-light-brown">{product.buod_reference}</p>}</div><button ref={initialFocusRef} type="button" onClick={onClose} aria-label={t('Close editor', 'إغلاق المحرر')} className="admin-icon-button"><X/></button></header>
    <div className="space-y-6 p-5 sm:p-7">{error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-700">{error}</div>}{success && <div role="status" className="flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 p-4 text-sm font-medium text-green-700"><CheckCircle2 size={17}/>{success}</div>}
...
export default function AdminBlocks() {
  ...
  return <AdminLayout title={t('Blocks', 'البلوكات')} subtitle={`${count} ${t('database records', 'سجلات قاعدة البيانات')}`}><div className="mb-5 flex flex-wrap items-center gap-3"><div className="relative min-w-64 flex-1"><Search size={15} className="absolute start-3 top-1/2 -translate-y-1/2 text-light-brown"/><input aria-label={t('Search blocks', 'البحث في البلوكات')} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('Search name, slug or BUOD reference…', 'ابحث بالاسم أو الرابط أو مرجع BUOD…')} className="input-field bg-white ps-9"/></div><select aria-label={t('Filter by publication state', 'تصفية حسب حالة النشر')} value={publicationState} onChange={(event) => setPublicationState(event.target.value)} className="input-field w-auto min-w-36 bg-white"><option value="">{t('All states', 'كل الحالات')}</option><option value="draft">{t('Draft', 'مسودة')}</option><option value="published">{t('Published', 'منشور')}</option><option value="archived">{t('Archived', 'مؤرشف')}</option></select><select aria-label={t('Filter by category', 'تصفية حسب الفئة')} value={categoryId} onChange={(event) => setCategoryId(event.target.value)} className="input-field w-auto min-w-44 bg-white"><option value="">{t('All categories', 'كل الفئات')}</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name_en} · {category.name_ar}</option>)}</select><input aria-label={t('Find a supplier to filter by', 'ابحث عن مورد للتصفية')} value={supplierSearch} onChange={(event) => setSupplierSearch(event.target.value)} placeholder={t('Find supplier…', 'ابحث عن مورد…')} className="input-field w-40 bg-white"/><select aria-label={t('Filter by supplier', 'تصفية حسب المورد')} value={supplierId} onChange={(event) => setSupplierId(event.target.value)} className="input-field w-auto max-w-56 bg-white"><option value="">{t('All suppliers', 'كل الموردين')}</option>{supplierOptions.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.company_name_en} · {supplier.company_name_ar}</option>)}</select><button type="button" onClick={load} className="admin-secondary-button"><RefreshCw size={14}/>{t('Refresh', 'تحديث')}</button><button type="button" onClick={() => openProduct(null)} className="admin-primary-button"><Plus size={15}/>{t('Create block', 'إنشاء بلوك')}</button></div>
    ...
    {editorLoading && <div role="status" aria-label={t('Loading block editor', 'جارٍ تحميل محرر البلوك')} className="fixed inset-0 z-50 grid place-items-center bg-deep-brown/70"><Loader2 aria-hidden="true" className="animate-spin text-gold" size={36}/></div>}{editor && <ProductEditor key={editor.id || 'new'} initial={editor} categories={categories} supplierOptions={supplierOptions} onClose={() => setEditor(null)} onSaved={() => load()} t={t}/>}
  </AdminLayout>;
}
