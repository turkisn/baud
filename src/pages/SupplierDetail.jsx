...
import { ArrowLeft, BadgeCheck, Building2, ExternalLink, Globe, Loader2, MapPin, Package } from 'lucide-react';
...
export default function SupplierDetail() {
  ...
  useEffect(() => {
    ...
    setError('');
    setProductsError('');
    mvpService.recordEvent('catalog_view', { page: 'supplier_detail' });
  ...
  return <div className="min-h-screen bg-ivory pb-20 pt-[76px]"><div className="relative h-72 overflow-hidden bg-black">{supplier.signed_cover_url && <img src={supplier.signed_cover_url} alt="" className="h-full w-full object-cover opacity-45"/>}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/35 to-transparent"/><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_100%,rgba(201,168,76,.18),transparent_48%)]"/></div><div className="mx-auto -mt-20 max-w-7xl px-6"><Link to="/suppliers" className="relative mb-5 inline-flex items-center gap-2 text-sm font-semibold text-warm-white hover:text-gold"><ArrowLeft className={lang === 'ar' ? 'rotate-180' : ''} size={16}/>{t('All suppliers', 'جميع الموردين')}</Link><section className="digital-panel relative rounded-3xl p-7 sm:p-10"><div className="flex flex-col gap-6 sm:flex-row sm:items-start"><div className="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-gold/25 bg-[#0b0a08] p-2">{supplier.signed_logo_url ? <img src={supplier.signed_logo_url} alt={`${name} logo`} className="h-full w-full object-contain"/> : <Building2 className="text-gold/35" size={42}/>}</div><div className="flex-1"><h1 className="text-3xl font-black text-warm-white sm:text-4xl">{name}</h1>{otherName && <p className="mt-1 text-sand/45">{otherName}</p>}<div className="mt-3 flex flex-wrap gap-2">{supplier.verification_status === 'verified' && <span className="inline-flex items-center gap-1 rounded-lg border border-gold/35 bg-gold/10 px-3 py-1 text-xs font-semibold text-light-gold"><BadgeCheck size={14}/>{t('Verified supplier', 'مورد موثق')}</span>}{supplier.product_count !== null && supplier.product_count !== undefined && <span className="inline-flex items-center gap-1 rounded-lg border border-gold/20 bg-black/50 px-3 py-1 text-xs text-sand"><Package size={14}/>{supplier.product_count} {t('published products', 'منتجات منشورة')}</span>}</div><p className="mt-5 max-w-3xl whitespace-pre-line leading-7 text-sand/65">{description || t('Supplier description unavailable.', 'وصف المورد غير متاح.')}</p><div className="mt-6 flex flex-wrap gap-4 text-sm">{location && <span className="flex items-center gap-2 text-sand/65"><MapPin className="text-gold" size={17}/>{location}</span>}{websiteUrl && <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-semibold text-gold hover:underline"><Globe size={17}/>{t('Visit website', 'زيارة الموقع')}<ExternalLink size={13}/></a>}</div></div></div></section>
