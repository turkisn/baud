...
export default function BlockDetail() {
  ...
  return <div className="min-h-screen bg-ivory pb-20 pt-[76px]">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-8">
      ...
      <div className="flex items-center gap-3"><span role="status" className="text-xs text-green-700">{shareNotice}</span><button type="button" onClick={sharePage} className="inline-flex items-center gap-2 rounded-xl border border-sand bg-white px-4 py-2 text-sm font-semibold text-medium-brown hover:border-gold"><Share2 size={15}/>{t('Share', 'مشاركة')}</button></div>
    </div>
    ...
    <div className="mx-auto mt-14 grid max-w-7xl gap-8 px-6 lg:grid-cols-2">
      <section className="rounded-3xl border border-sand bg-white p-7"><h2 className="mb-5 flex items-center gap-2 text-xl font-bold text-dark-brown"><Box className="text-gold"/>{t('Specifications', 'المواصفات')}</h2>{detailErrors.specifications ? <SectionError>{t('Specifications could not be loaded.', 'تعذّر تحميل المواصفات.')}</SectionError> : product.product_specifications.length ? <dl>{product.product_specifications.map((spec) => <DataValue key={spec.id} label={lang === 'ar' ? spec.specification_name_ar || spec.specification_name_en : spec.specification_name_en || spec.specification_name_ar} value={[spec.value, spec.unit].filter(Boolean).join(' ')}/>)}</dl> : <p className="text-sm text-light-brown">{t('Specifications unavailable.', 'المواصفات غير متاحة.')}</p>}</section>
